...
int fmincg(void (*costFunc)(COST_FUNC_DATATYPE* inputVector, COST_FUNC_DATATYPE* cost, COST_FUNC_DATATYPE* gradVector, int* pari, double *pard), COST_FUNC_DATATYPE* xVector, int* pari, double *pard, int nDim, int maxCostCalls, double &f1, double *df1)
{
	...
	COST_FUNC_DATATYPE /*df1[nDimA],*/s[nDimA],x0[nDimA],df0[nDimA],df2[nDimA],tmp[nDimA];
	COST_FUNC_DATATYPE* x = xVector;

	ls_failed = 0;
	
	if(costFuncCount >= maxCostCalls) return 1; else costFuncCount++;
	(*costFunc)(xVector,&f1,df1,pari,pard);
